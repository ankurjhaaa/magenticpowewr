<div>
    <div class="flex items-center justify-between gap-3 mb-6">
        <button
            type="button"
            @click="$dispatch('open-offcanvas', { name: 'brand-filters' })"
    ...
    {{-- Desktop table --}}
    <x-admin.table :headers="['Brand', 'Products', 'Status', 'Sort', 'Actions']">
        @forelse ($brands as $brand)
            <tr wire:key="row-{{ $brand->id }}">
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3">
                        @if ($brand->logo)
                            <img src="{{ Storage::url($brand->logo) }}" class="w-10 h-10 rounded-lg object-cover border border-gray-300">
                        @else
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-600">{{ $brand->slug }}</p>
                        </div>
                    </div>
                </td>
                <td class="px-4 py-3 text-sm text-gray-700">{{ $brand->products_count }}</td>
                <td class="px-4 py-3">
                    @if ($brand->trashed())
                    ...
                    @else
                        <button wire:click="toggleStatus({{ $brand->id }})"
                            class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium cursor-pointer {{ $brand->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                            {{ $brand->is_active ? 'Active' : 'Inactive' }}
                        </button>
                    @endif
                </td>
                <td class="px-4 py-3 text-sm text-gray-700">{{ $brand->sort_order }}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3 text-sm">
                        @if ($brand->trashed())
                            <button wire:click="restore({{ $brand->id }})" class="text-gray-700 hover:text-gray-900 font-semibold cursor-pointer">Restore</button>
                        @else
                            <button wire:click="edit({{ $brand->id }})" class="text-gray-700 hover:text-gray-900 font-semibold cursor-pointer">Edit</button>
                            <button wire:click="confirmDelete({{ $brand->id }})" class="text-red-600 hover:text-red-700 font-semibold cursor-pointer">Delete</button>
                        @endif
                    </div>
                </td>
            </tr>
        @empty
            <tr>
                <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-600">No brands found.</td>
            </tr>
        @endforelse
    </x-admin.table>
    ...
    <x-admin.card-list>
        @forelse ($brands as $brand)
            <x-admin.card wire:key="card-{{ $brand->id }}">
                <div class="flex items-center gap-3">
                    @if ($brand->logo)
                        <img src="{{ Storage::url($brand->logo) }}" class="w-12 h-12 rounded-lg object-cover border border-gray-300">
                    @else
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-600 truncate">{{ $brand->slug }}</p>
                    </div>
                    @if ($brand->trashed())
                        <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600">Trashed</span>
                    @else
                ...
                <div class="flex items-center justify-between pt-2 border-t border-gray-300 text-sm">
                    <span class="text-gray-700">{{ $brand->products_count }} products &middot; sort {{ $brand->sort_order }}</span>
                    <div class="flex items-center gap-3">
                        @if ($brand->trashed())
                            <button wire:click="restore({{ $brand->id }})" class="text-gray-700 font-semibold cursor-pointer">Restore</button>
                        @else
                            <button wire:click="edit({{ $brand->id }})" class="text-gray-700 font-semibold cursor-pointer">Edit</button>
                            <button wire:click="confirmDelete({{ $brand->id }})" class="text-red-600 font-semibold cursor-pointer">Delete</button>
                        @endif
                    </div>
                </div>
            </x-admin.card>
    ...
    {{-- Filters offcanvas --}}
    <x-admin.offcanvas name="brand-filters" title="Filters">
        <div class="space-y-4">
            <div>
    ...
    >
        <form wire:submit="save" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model="website_url" placeholder="https://example.com" class="block w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-gray-900">
            ...
            <div>
                ...
                @if ($existingLogo && ! $logo)
                    <img src="{{ Storage::url($existingLogo) }}" class="w-16 h-16 rounded-lg object-cover border border-gray-200 mb-2">
                @endif
    ...
    <x-admin.modal name="delete-brand" max-width="sm">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Delete brand?</h3>
        <p class="text-sm text-gray-500 mb-6">This brand will be moved to trash. You can restore it later using the "Show trashed" filter.</p>

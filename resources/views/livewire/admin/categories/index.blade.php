<div>
    <div class="flex items-center justify-between gap-3 mb-6">
        <button
            type="button"
            @click="$dispatch('open-offcanvas', { name: 'category-filters' })"
    ...
    {{-- Desktop table --}}
    <x-admin.table :headers="['Category', 'Products', 'Status', 'Sort', 'Actions']">
        @forelse ($categories as $category)
            <tr wire:key="row-{{ $category->id }}">
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3">
                        @if ($category->image)
                            <img src="{{ Storage::url($category->image) }}" class="w-10 h-10 rounded-lg object-cover border border-gray-300">
                        @else
                            <div class="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 text-xs">N/A</div>
                        @endif
                        <div>
                            <p class="text-sm font-medium text-gray-900">{{ $category->name }}</p>
                            <p class="text-xs text-gray-600">{{ $category->slug }}</p>
                        </div>
                    </div>
                </td>
                ...
                <td class="px-4 py-3">
                    ...
                    @else
                        <button wire:click="toggleStatus({{ $category->id }})"
                            class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium cursor-pointer {{ $category->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                ...
                <td class="px-4 py-3">
                    <div class="flex items-center gap-3 text-sm">
                        @if ($category->trashed())
                            <button wire:click="restore({{ $category->id }})" class="text-gray-700 hover:text-gray-900 font-semibold cursor-pointer">Restore</button>
                        @else
                            <button wire:click="edit({{ $category->id }})" class="text-gray-700 hover:text-gray-900 font-semibold cursor-pointer">Edit</button>
                            <button wire:click="confirmDelete({{ $category->id }})" class="text-red-600 hover:text-red-700 font-semibold cursor-pointer">Delete</button>
                        @endif
                    </div>
                </td>
            </tr>
        @empty
            <tr>
                <td colspan="5" class="px-4 py-10 text-center text-sm text-gray-600">No categories found.</td>
            </tr>
        @endforelse
    </x-admin.table>
    ...
    <x-admin.card-list>
        @forelse ($categories as $category)
            <x-admin.card wire:key="card-{{ $category->id }}">
                <div class="flex items-center gap-3">
                    @if ($category->image)
                        <img src="{{ Storage::url($category->image) }}" class="w-12 h-12 rounded-lg object-cover border border-gray-300">
                    @else
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-xs text-gray-600 truncate">{{ $category->slug }}</p>
                    </div>
                    @if ($category->trashed())
                        <span class="inline-flex items-center rounded-full bg-gray-100 px-2.5 py-0.5 text-xs font-medium text-gray-600">Trashed</span>
                    @else
                ...
                <div class="flex items-center justify-between pt-2 border-t border-gray-300 text-sm">
                    <span class="text-gray-700">{{ $category->products_count }} products &middot; sort {{ $category->sort_order }}</span>
                    <div class="flex items-center gap-3">
                        @if ($category->trashed())
                            <button wire:click="restore({{ $category->id }})" class="text-gray-700 font-semibold cursor-pointer">Restore</button>
                        @else
                            <button wire:click="edit({{ $category->id }})" class="text-gray-700 font-semibold cursor-pointer">Edit</button>
                            <button wire:click="confirmDelete({{ $category->id }})" class="text-red-600 font-semibold cursor-pointer">Delete</button>
                        @endif
                    </div>
                </div>
            </x-admin.card>
    ...
    {{-- Filters offcanvas --}}
    <x-admin.offcanvas name="category-filters" title="Filters">
        <div class="space-y-4">
            <div>
        ...
        <form wire:submit="save" class="space-y-4">
            ...
            <div>
                ...
                @if ($existingImage && ! $image)
                    <img src="{{ Storage::url($existingImage) }}" class="w-16 h-16 rounded-lg object-cover border border-gray-200 mb-2">
                @endif
    ...
    <x-admin.modal name="delete-category" max-width="sm">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Delete category?</h3>
        <p class="text-sm text-gray-500 mb-6">This category will be moved to trash. You can restore it later using the "Show trashed" filter.</p>
